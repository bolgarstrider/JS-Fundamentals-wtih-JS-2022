...
// or the original matrix if the two diagonals have different sums. 
// You need to print every row on a new line, with cells separated by a space. Check the examples below. 
...
function diagonalsAttack(matrixRows) {
    ...
    for (let i = 0; i < matrixRows.length; i++) {
        matrixToNumber.push(matrixRows[i].split(' '));
    }


    let matrix = [];
    for (let i = 0; i < matrixToNumber.length; i++) {
        let row = [];

        for (let j = 0; j < matrixToNumber.length; j++) {

            row.push(Number(matrixToNumber[i][j]));
        }
